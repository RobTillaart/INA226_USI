...
//
// Modified for use with ATtiny85 by J.P.Simoons - 8 augustus 2026 - mod 2 lines
...
//  CONSTRUCTOR
//
//INA226::INA226(const uint8_t address, TwoWire *wire)                     // J.P.S.: replaced by next line for ATtiny85 (1)
INA226::INA226(const uint8_t address, USI_TWI *wire)
{
  _address     = address;
  _wire        = wire;
...
uint16_t INA226::_readRegister(uint8_t reg)
{
  ...
  uint16_t value = 0;
 // if (2 == _wire->requestFrom(_address, (uint8_t)2))             // J.P.S.: replaced by next line for ATtiny85 (2)
  if (0 == _wire->requestFrom(_address, (uint8_t)2))               // TWI_USI.requestFrom() allway returns a completion code, not the number of byes read
  {
    value = _wire->read();
    value <<= 8;
    value |= _wire->read();
